...
csvOutputFile = ""
jsonOutputFile = ""
checkLicenses = False  # Do a license comparison?
# =============================================
...
comparisonData = {
    ...
    "componentMissedBySonatype": [],
    "componentMissedBySonatypeLength": 0,
    "potentialMatches": [],
    "potentialMatchesLength": 0,
    "cveFoundBySonatype": 0,
    "sonatypeProprietaryCVECount": 0,
    "competitorProprietaryCVECount": 0,
    "cveFoundByCompetitor": 0,
    "cveFoundByBoth": 0,
    "cveMissingCompetitor": [],
    "cveMissingCompetitorLength": 0,
    "cveMissingSonatype": [],
    "cveMissingSonatypeLength": 0,
...
def import_reports():
    ...
    for i in jsonData:
        if i["securityData"] != None:
            ...
            # Get licenses
             #Get licenses
            licData = get_licence_array(i["licenseData"])
            lics = licData["lics"]
    ...
    # If file is .json
        ...
        for i in jsonData["components"]:
            ...
            if len(i["version"]) > 0:
                ...
                cves = []
                for j in jsonData["vulnerabilities"]:
                    for k in j["affects"]:
        ...
        print("ERROR READING COMPETITOR SBOM. ", competitorSBOM, " NOT COMPATIBLE!")
        quit()
...
def get_licence_array(e):
    ...
    returnLics = []
    for i in licenses:
        if i not in returnLics:
            returnLics.append(i)

    # Get license threats
    threats = []
    for i in e["effectiveLicenseThreats"]:
        if i["licenseThreatGroupLevel"] >=7:
            threats.append(i["licenseThreatGroupName"])
...
def compare_components():
 ...
 #               print("Stop here!!")
        ...
        if found == False and i["component"] not in comparisonData["componentMissedBySonatype"]:
            comparisonData["componentMissedBySonatype"].append(i["component"])
            csvReportFormat.append({
                "sonaName": "",
                "sonaCVEs": "",
                "sonaPath": "",
    ...
    comparisonData["componentMissedBySonatype"].sort()
    comparisonData["componentMissedBySonatypeLength"] = len(comparisonData["componentMissedBySonatype"])

    print("Sonatype found ", comparisonData["componentMissedByCompetitorLength"],
          " components not found by the compeitor.")
    print("The competitor found ", comparisonData["componentMissedBySonatypeLength"], " not found by Sonatype.")
...
def fuzzy_match_components():
    ...
    compMissing = comparisonData["componentMissedBySonatype"]  # comparisonData["componentMissedByCompetitor"]
    # sonaMissing = comparisonData["componentMissedBySonatype"]
...
def compare_cves():
    ...
    for comp in competitorData:
        for comp_cve in comp["cve"]:
            ...
            if found == False:
                comparisonData["cveMissingSonatype"].append(comp_cve + ", " + comp["component"])
    ...
    for i in range(len(comparisonData["cveMissingSonatype"])):
        for key in uniqueCVECount:
            if key in comparisonData["cveMissingSonatype"][i]:
                repl = key + ", (" + str(uniqueCVECount[key]) + ")"
                comparisonData["cveMissingSonatype"][i] = comparisonData["cveMissingSonatype"][i].replace(
                    key, repl)
                break

    # print(comparisonData["cveMissingSonatype"])
    comparisonData["cveMissingSonatypeLength"] = len(comparisonData["cveMissingSonatype"])

    print("Getting CVE Counts...")
    cveList = {
        "sonatype": [],
        "competitor": []
    }
    ...
    for i in csvReportFormat:
        if header > 0:
            cveList["sonatype"] += i["sonaCVEs"]
            cveList["competitor"] += i["compCVEs"]
        else:
            header = 1

    # CVE Count
    comparisonData["cveFoundBySonatype"] = str(len(cveList["sonatype"])) +" ("+str(len(set(cveList["sonatype"])))+" Unique)"
    comparisonData["cveFoundByCompetitor"] = str(len(cveList["competitor"]))+" ("+str(len(set(cveList["competitor"])))+" Unique)"
    uniqueMatches = 0

    for i in cveList["sonatype"]:
        if "CVE" not in i:
            comparisonData["sonatypeProprietaryCVECount"] += 1

    for i in cveList["competitor"]:
        if "CVE" not in i:
            comparisonData["competitorProprietaryCVECount"] += 1

    # Count unique CVEs
    cveList["sonatype"] = set(cveList["sonatype"])
    cveList["competitor"] = set(cveList["competitor"])
    for i in cveList["sonatype"]:
        if i in cveList["competitor"]:
            uniqueMatches += 1
    comparisonData["cveFoundByBoth"] = str(comparisonData["cveFoundByBoth"])+" including duplicates ("+str(uniqueMatches)+" Unique)"
    ...
    for i in csvReportFormat:
        ...
        if (i["match"] == "" and
            ...
            for j in sonatypeData:
                if i["sonaName"] == j["component"]:
                    if j["licenseThreats"] != []:
                        licIssues.append(str(j["licenseThreats"])+" : "+j["component"])
...
def main(e):
    ...
    global competitorSBOM, sonatypeSBOM, csvOutputFile, jsonOutputFile
