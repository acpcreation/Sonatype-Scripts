...
useArtifactoryRawOutputCache = True # Local raw-out.json
typeIdentifiers = [
    {
        "extension":".jar",
        "type":"maven"
...
def get_nexus_components():
    ...
    res = requests.get(url+"service/rest/v1/search?repository="+auditRepository+"&maven.extension=jar", auth=(username, password)) #, timeout=120
    if res.text != "":
        ...
        while contToken != None:
            ...
            res = requests.get(url+"service/rest/v1/search?repository="+auditRepository+"&continuationToken="+contToken+"&maven.extension=jar", auth=(username, password)) #, timeout=120
            # print(res.text)
            res = json.loads(res.text)
            components = components + res["items"]
            contToken = res["continuationToken"]
